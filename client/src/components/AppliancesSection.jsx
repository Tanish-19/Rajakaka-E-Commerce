...
// ApplianceCard Component
function ApplianceCard({ appliance }) {
  const navigate = useNavigate();

  const handleCardClick = () => {
    navigate(`/product/${appliance.product_id || appliance._id}`);
  };

  const handleAddToCart = (e) => {
    e.stopPropagation();
    console.log('Product', appliance.name, 'added to cart.');
  };
  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-gray-50 flex items-center justify-center overflow-hidden">
        <img
          src={appliance.images?.[0] || '/placeholder.png'}
          alt={appliance.name}
      ...
      <div className="p-4 flex flex-col flex-grow">
        <h3 className="font-bold text-gray-800 text-lg mb-2 line-clamp-2">{appliance.name}</h3>

        <div className="space-y-1 mb-3 text-sm text-gray-600">
          {appliance.storage && appliance.storage.length > 0 && (
            <div className="flex items-center gap-2">
              <span className="font-medium">Capacity:</span>
              <span className="line-clamp-1">{appliance.storage[0]}</span>
            </div>
          )}
          {appliance.brand && (
            <div className="flex items-center gap-2">
          ...
          {appliance.colors && appliance.colors.length > 0 && (
            <div className="flex items-center gap-2">
              <span className="font-medium">Colors:</span>
              <span>{appliance.colors.slice(0, 2).join(', ')}</span>
            </div>
          )}
        </div>
        ...
        <div className="mt-auto">
          <div className="flex items-baseline gap-2 mb-1">
            <span className="text-2xl font-bold text-orange-600">₹{appliance.price?.toLocaleString()}</span>
            {appliance.original_price && appliance.original_price > appliance.price && (
              <span className="text-sm text-gray-500 line-through">₹{appliance.original_price.toLocaleString()}</span>
            )}
          </div>

          {appliance.discount > 0 && appliance.original_price && (
            <p className="text-xs text-green-600 font-semibold mb-3">
              Save ₹{(appliance.original_price - appliance.price).toLocaleString()}
            </p>
          )}
...
// ApplianceListingPage Component
function ApplianceListingPage() {
  ...
  const fetchAppliances = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}?category=appliances`);
      const data = await response.json();

      if (data.success) {
        setAllAppliances(data.data);
      } else {
  ...
  const brands = [...new Set(allAppliances.map(app => app.brand).filter(Boolean))].sort();

  const filteredAppliances = allAppliances.filter(appliance => {
    const brandMatch = selectedBrand === 'All' || appliance.brand === selectedBrand;
      ...
      <div className="bg-gray-100 min-h-screen">
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600 font-medium">Loading appliances...</p>
          </div>
        </div>
      </div>
    );
